...
The heartbeat is a **safety net**, not the primary detection mechanism. Tasks have their own `monitor_pid()` watchdog that detects death within ~2 minutes and sends Discord notifications directly. The 5-minute `monitor_task` cron provides a second layer. This heartbeat (every 30 minutes) catches anything that slipped through both layers.
...
```bash
~/.openclaw/skills/long-running-task/bin/monitor_task --channel discord
```

Then run the full cleanup (dead PIDs, stall warnings, pruning, worktree cleanup):

```bash
~/.openclaw/skills/long-running-task/bin/cleanup_tasks --channel discord
```

## 2. Handle completed/failed tasks

If the cleanup script output contains `=== TASK COMPLETED:` or `=== TASK FAILED:` blocks, compose a notification for each task and **output it as your final response text**. Do NOT use the `message` tool — your response is automatically delivered to Discord by the heartbeat system.
...
- Use Discord-friendly formatting (bold, bullet lists — no markdown tables)
- Keep each notification to 3-5 bullet points max

## 3. Handle monitorDetected tasks

If a failed task has `monitorDetected: true` in its note or manifest, it means the monitor already sent a Discord notification to the user. Do NOT re-notify. Instead:
